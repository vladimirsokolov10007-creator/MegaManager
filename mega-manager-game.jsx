...
function GameScreen({ player, character, onComplete }) {
  const [gameState, setGameState] = useState({
    ...
    xp: 0,
    missionIndex: 0,
    phase: "briefing", // briefing, assign, result, shop, event
    employees: [
  ...
  // Calculate score for current assignment
  ...
  const submitAssignment = () => {
    ...
    // Update stress
    const newEmployees = gs.employees.map((emp) => {
    ...
    if (passed) {
      ...
      update({
        ...
        megaMsg: `🎉 Проект "${mission.title}" выполнен! Метрика: ${totalMetric.toFixed(1)}/${mission.threshold}. +${mission.reward.money}к 💰 +${mission.reward.social} соц. балл!`,
        notifications: [],
      });
    } else {
      const reason = failed ? failReason : `Метрика ${totalMetric.toFixed(1)} < ${mission.threshold}. Нужно перераспределить команду!`;
      update({
        megaMsg: `❌ Проект провален! ${reason}`,
        assignments: { 1: [], 2: [], 3: [] },
    ...
    return (
      <div style={{ ...S.app, padding: "16px", overflowY: "auto", background: "linear-gradient(180deg, #0a0a2e, #0c1a3e)" }}>
        ...
        <div style={{ maxWidth: 900, margin: "72px auto 0" }}>
          <div style={{
            fontSize: 14, textAlign: "center", marginBottom: 16, opacity: 0.7,
          }}>
            📋 {mission.title} — {mission.metric === "speed" ? "⚡ СКОРОСТЬ" : "💎 КАЧЕСТВО"} ≥ {mission.threshold}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 12, marginBottom: 20 }}>
            {ROOMS.map((room) => (
              <div key={room.id} style={{
                ...S.card, padding: 16,
                border: gs.currentRoom === room.id ? `2px solid ${character.color}` : "1px solid rgba(255,255,255,0.06)",
              }}>
                ...
                <div style={{ display: "flex", gap: 6, minHeight: 50, flexWrap: "wrap" }}>
                  ...
                  {gs.assignments[room.id].length === 0 && (
                    <div style={{ fontSize: 12, opacity: 0.3, padding: 8 }}>Перетащи сюда сотрудников</div>
                  )}
                </div>
              </div>
            ))}
          ...
          <div style={{ ...S.card, padding: 16, marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>👥 Команда (нажми → выбери комнату)</div>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            ...
            <button onClick={() => update({ assignments: { 1: [], 2: [], 3: [] } })} style={S.btn("#666", true)}>
              🔄 Сбросить
            </button>
          </div>
        </div>
...
function HUD({ gs, player, character }) {
  return (
    <div style={{
      ...
      <div style={{ display: "flex", gap: 14, marginLeft: "auto", fontSize: 13 }}>
